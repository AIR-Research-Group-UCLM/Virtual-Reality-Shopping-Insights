...
/// <summary>
/// Manages tracking and saving of shopping cart data in batches.
/// </summary>
public class ShoppingCartTrackerManager : MonoBehaviour
{
    ...
    /// <summary>
    /// File path for saving the shopping cart data.
    /// </summary>
    ...
    /// <summary>
    /// Adds shopping cart data to the batch list.
    /// </summary>
    /// <param name="data">The shopping cart data to add following the structure of the csv headers.</param>
    ...
    void AddCsvHeader()
    {
        if (!File.Exists(filePath))
        {
            using (StreamWriter writer = new StreamWriter(filePath, true))
            {
                writer.WriteLine("Frame, Timestamp, Item, Action");
            }
        }
    }
